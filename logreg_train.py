...
def compute_gradients(X, y, y_hat):
    m = len(y)
    dw = (1/m) * np.dot(X.T, (y_hat - y)) # X.T: feature matrix (y_hat - y): column vector of prediction errors
...
def train_one_house(X, y, house_name, learning_rate=0.01, epochs=1000):
    """
    Train weights for one house using gradient descent
    X: feature matrix (students × features)
    y: actual house labels
    house_name: which house we're training for
    """
    ...
    for i in range(epochs):
        ...
        if i % 100 == 0 or i == epochs - 1:
            print(f"{house_name} Epoch {i + 1}|{epochs}: loss = {loss:.4f}") 
...
if __name__ == "__main__":
    ...
    for house in HOUSES:
        ...
        # Train
        w, b, loss_history = train_one_house(
            X, y_binary, house, 
        ...
        plt.savefig(f'loss_history_{house}.png')
        plt.close()
        print(f"Please see the loss history @ loss_history_{house}.png")
